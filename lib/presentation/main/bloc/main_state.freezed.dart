// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'main_state.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#adding-getters-and-methods-to-our-models');

/// @nodoc
mixin _$MainState {
  ViewState get state => throw _privateConstructorUsedError;
  double get progress => throw _privateConstructorUsedError;
  int get totalSecs => throw _privateConstructorUsedError;
  String? get formattedDuration => throw _privateConstructorUsedError;
  Uint8List? get snapshot => throw _privateConstructorUsedError;

  @JsonKey(ignore: true)
  $MainStateCopyWith<MainState> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $MainStateCopyWith<$Res> {
  factory $MainStateCopyWith(MainState value, $Res Function(MainState) then) =
      _$MainStateCopyWithImpl<$Res, MainState>;
  @useResult
  $Res call(
      {ViewState state,
      double progress,
      int totalSecs,
      String? formattedDuration,
      Uint8List? snapshot});
}

/// @nodoc
class _$MainStateCopyWithImpl<$Res, $Val extends MainState>
    implements $MainStateCopyWith<$Res> {
  _$MainStateCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? state = null,
    Object? progress = null,
    Object? totalSecs = null,
    Object? formattedDuration = freezed,
    Object? snapshot = freezed,
  }) {
    return _then(_value.copyWith(
      state: null == state
          ? _value.state
          : state // ignore: cast_nullable_to_non_nullable
              as ViewState,
      progress: null == progress
          ? _value.progress
          : progress // ignore: cast_nullable_to_non_nullable
              as double,
      totalSecs: null == totalSecs
          ? _value.totalSecs
          : totalSecs // ignore: cast_nullable_to_non_nullable
              as int,
      formattedDuration: freezed == formattedDuration
          ? _value.formattedDuration
          : formattedDuration // ignore: cast_nullable_to_non_nullable
              as String?,
      snapshot: freezed == snapshot
          ? _value.snapshot
          : snapshot // ignore: cast_nullable_to_non_nullable
              as Uint8List?,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$MainStateImplCopyWith<$Res>
    implements $MainStateCopyWith<$Res> {
  factory _$$MainStateImplCopyWith(
          _$MainStateImpl value, $Res Function(_$MainStateImpl) then) =
      __$$MainStateImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {ViewState state,
      double progress,
      int totalSecs,
      String? formattedDuration,
      Uint8List? snapshot});
}

/// @nodoc
class __$$MainStateImplCopyWithImpl<$Res>
    extends _$MainStateCopyWithImpl<$Res, _$MainStateImpl>
    implements _$$MainStateImplCopyWith<$Res> {
  __$$MainStateImplCopyWithImpl(
      _$MainStateImpl _value, $Res Function(_$MainStateImpl) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? state = null,
    Object? progress = null,
    Object? totalSecs = null,
    Object? formattedDuration = freezed,
    Object? snapshot = freezed,
  }) {
    return _then(_$MainStateImpl(
      state: null == state
          ? _value.state
          : state // ignore: cast_nullable_to_non_nullable
              as ViewState,
      progress: null == progress
          ? _value.progress
          : progress // ignore: cast_nullable_to_non_nullable
              as double,
      totalSecs: null == totalSecs
          ? _value.totalSecs
          : totalSecs // ignore: cast_nullable_to_non_nullable
              as int,
      formattedDuration: freezed == formattedDuration
          ? _value.formattedDuration
          : formattedDuration // ignore: cast_nullable_to_non_nullable
              as String?,
      snapshot: freezed == snapshot
          ? _value.snapshot
          : snapshot // ignore: cast_nullable_to_non_nullable
              as Uint8List?,
    ));
  }
}

/// @nodoc

class _$MainStateImpl implements _MainState {
  _$MainStateImpl(
      {this.state = ViewState.initial,
      this.progress = 0,
      this.totalSecs = 0,
      this.formattedDuration,
      this.snapshot});

  @override
  @JsonKey()
  final ViewState state;
  @override
  @JsonKey()
  final double progress;
  @override
  @JsonKey()
  final int totalSecs;
  @override
  final String? formattedDuration;
  @override
  final Uint8List? snapshot;

  @override
  String toString() {
    return 'MainState(state: $state, progress: $progress, totalSecs: $totalSecs, formattedDuration: $formattedDuration, snapshot: $snapshot)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$MainStateImpl &&
            (identical(other.state, state) || other.state == state) &&
            (identical(other.progress, progress) ||
                other.progress == progress) &&
            (identical(other.totalSecs, totalSecs) ||
                other.totalSecs == totalSecs) &&
            (identical(other.formattedDuration, formattedDuration) ||
                other.formattedDuration == formattedDuration) &&
            const DeepCollectionEquality().equals(other.snapshot, snapshot));
  }

  @override
  int get hashCode => Object.hash(runtimeType, state, progress, totalSecs,
      formattedDuration, const DeepCollectionEquality().hash(snapshot));

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$MainStateImplCopyWith<_$MainStateImpl> get copyWith =>
      __$$MainStateImplCopyWithImpl<_$MainStateImpl>(this, _$identity);
}

abstract class _MainState implements MainState {
  factory _MainState(
      {final ViewState state,
      final double progress,
      final int totalSecs,
      final String? formattedDuration,
      final Uint8List? snapshot}) = _$MainStateImpl;

  @override
  ViewState get state;
  @override
  double get progress;
  @override
  int get totalSecs;
  @override
  String? get formattedDuration;
  @override
  Uint8List? get snapshot;
  @override
  @JsonKey(ignore: true)
  _$$MainStateImplCopyWith<_$MainStateImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
